...
from back.insertar import agregar_silo
from back.data import leer_usuario
import interfaz
...
class Medida:
    ...
    def __init__(self):
        ...
        saludo = tk.Label(self.window, text="Aquí puedes agregar un nuevo silo!!", borderwidth=2,bg='#69DADB', fg='white',  font='arial 25', height=2, width=100)
        saludo.grid(row=0, column=0, padx=5, pady=30) 



        """PANEL PARA INGRESAR DATOS"""
        panel_principal = tk.Frame(self.window, borderwidth=2,height=400, width=900, bg='#1597E5')
        panel_principal.grid(row=1, column=0, pady=30)

        tk.Label(panel_principal, text="Capacidad:", font='arial 14', bg='#1597E5').grid(pady=10, row=0, column=0)
        tk.Label(panel_principal, text="Altura:", font='arial 14', bg='#1597E5').grid(pady=10, row=1, column=0)
        tk.Label(panel_principal, text="Diámetro:", font='arial 14', bg='#1597E5').grid( pady=10, row=2, column=0)
        tk.Label(panel_principal, text="Altura del cono:", font='arial 14', bg='#1597E5').grid( pady=10, row=3, column=0)
    ...
    def aceptar(self):
        ...
        if self.verificacion() and self.validar():
            ...
            respuesta = agregar_silo(alturaCilindro, diametro, alturaCono,capacidad, usuario )
            if (respuesta ==0):
                messagebox.showinfo(message="Silo agregado",title="Mensaje")
                self.window.destroy()
                Medida()
            else:
                messagebox.showerror(message="No se pudo agregar el silo",title="Mensaje") 
        else:
